...
public class ConcreteCompareVisitor implements ConcreteExpressionVisitor<Concrete.Expression, Boolean>, ConcreteDefinitionVisitor<Concrete.Definition, Boolean> {
  ...
  public static boolean compare(Concrete.ReferableDefinition def1, Concrete.ReferableDefinition def2) {
    ...
    if (def1 instanceof Concrete.ClassFieldSynonym) {
      return def2 instanceof Concrete.ClassFieldSynonym && visitor.compareFieldSynonym((Concrete.ClassFieldSynonym) def1, (Concrete.ClassFieldSynonym) def2);
    }
    return false;
  }
  ...
  @Override
  public Boolean visitClassSynonym(Concrete.ClassSynonym def, Concrete.Definition def2) {
    if (!(def2 instanceof Concrete.ClassSynonym)) {
      return false;
    }
    Concrete.ClassSynonym synonym2 = (Concrete.ClassSynonym) def2;
    if (!compareExpressionList(def.getSuperClasses(), synonym2.getSuperClasses())) {
      return false;
    }
    if (!compare(def.getUnderlyingClass(), synonym2.getUnderlyingClass())) {
      return false;
    }
    if (def.getFields().size() != synonym2.getFields().size()) {
      return false;
    }
    for (int i = 0; i < def.getFields().size(); i++) {
      if (!compareFieldSynonym(def.getFields().get(i), synonym2.getFields().get(i))) {
        return false;
      }
    }
    return true;
  ...
  private boolean compareFieldSynonym(Concrete.ClassFieldSynonym field1, Concrete.ClassFieldSynonym field2) {
    mySubstitution.put(field1.getData(), field2.getData());
    return compare(field1.getUnderlyingField(), field2.getUnderlyingField());
  }
