...
public abstract class NameResolverTestCase extends ParserTestCase {
  ...
  private void resolveNamesModule(ChildGroup group, int errors) {
    ...
    libraryManager.getInstanceProviderSet().collectInstances(group, CachingScope.make(ScopeFactory.parentScopeForGroup(group, libraryManager.getModuleScopeProvider(), true)), ConcreteReferableProvider.INSTANCE);
    assertThat(errorList, containsErrors(errors));
  }
