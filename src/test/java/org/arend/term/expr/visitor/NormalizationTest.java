...
public class NormalizationTest extends TypeCheckingTestCase {
  ...
  public void initialize() {
    typeCheckModule(
      "\\func \\infixl 6 + (x y : Nat) : Nat \\elim x | 0 => y | suc x => suc (x + y)\n" +
      "\\func \\infixl 7 * (x y : Nat) : Nat \\elim x | 0 => 0 | suc x => y + x * y\n" +
      "\\func fac (x : Nat) : Nat | 0 => 1 | suc x => suc x * fac x\n" +
      "\\func nelim (z : Nat) (s : Nat -> Nat -> Nat) (x : Nat) : Nat \\elim x | 0 => z | suc x => s x (nelim z s x)");
