...
public class Prelude extends Namespace {
  ...
  static {
    ...
    DependentLink PathParameter1 = param("A", Pi(param(Interval()), Universe(new LevelExpression(LP), new LevelExpression(LH))));
    ...
    Preprelude.DefinitionBuilder.Data path = new Preprelude.DefinitionBuilder.Data(PRELUDE, "Path", Abstract.Binding.DEFAULT_PRECEDENCE, new TypeUniverse(new LevelExpression(LP), new LevelExpression(LH)), PathParameter1, Arrays.asList(LP, LH));
    ...
    PATH_CON = path.addConstructor("path", Abstract.Binding.DEFAULT_PRECEDENCE, new TypeUniverse(new LevelExpression(LP), new LevelExpression(LH)), pathParameter);

    /* = */
    // Binding pathInfixLp = new TypedBinding("lp", Lvl());
    // Binding pathInfixLh = new TypedBinding("lh", CNat());
    DependentLink pathInfixParameter1 = param(false, "A", Universe(new LevelExpression(LP), new LevelExpression(LH)));
    ...
    PATH_INFIX = new Preprelude.DefinitionBuilder.Function(PRELUDE, "=", new Abstract.Binding.Precedence(Abstract.Binding.Associativity.NON_ASSOC, (byte) 0), pathInfixParameter1, Universe(new LevelExpression(LP), new LevelExpression(LH)), top(pathInfixParameter1, leaf(pathInfixTerm)), Arrays.asList(LP, LH)).definition();

    /* @ */
    //Binding atLp = new TypedBinding("lp", Lvl());
    //Binding atLh = new TypedBinding("lh", CNat());
    DependentLink atParameter1 = param(false, "A", Pi(param(Interval()), Universe(new LevelExpression(LP), new LevelExpression(LH))));
    ...
    DependentLink coerceParameter1 = param("type", Pi(param(Interval()), Universe(new LevelExpression(LP), new LevelExpression(LH))));
    ...
    DependentLink isoParameter1 = param(false, vars("A", "B"), Universe(new LevelExpression(LP), new LevelExpression(LH)));
    ...
    Expression isoResultType = Universe(new LevelExpression(LP), new LevelExpression(LH));
    ...
    DependentLink truncParameter = param("A", Universe(new LevelExpression(LP), new LevelExpression(LH)));
    Preprelude.DefinitionBuilder.Data propTrunc = new Preprelude.DefinitionBuilder.Data(PRELUDE, "TrP", Abstract.Binding.DEFAULT_PRECEDENCE, TypeUniverse.PROP, truncParameter, Arrays.asList(LP, LH));
    PROP_TRUNC = propTrunc.definition();
    propTrunc.addConstructor("inP", Abstract.Binding.DEFAULT_PRECEDENCE, new TypeUniverse(new LevelExpression(LP), new LevelExpression(LH)), param("a", Reference(truncParameter)));
    ...
    PROP_TRUNC_PATH_CON = propTrunc.addConstructor("truncP", Abstract.Binding.DEFAULT_PRECEDENCE, new TypeUniverse(new LevelExpression(LP), new LevelExpression(LH)), propTruncConParameter1);
    ...
    /* TrS, inS */
    Preprelude.DefinitionBuilder.Data setTrunc = new Preprelude.DefinitionBuilder.Data(PRELUDE, "TrS", Abstract.Binding.DEFAULT_PRECEDENCE, TypeUniverse.SetOfLevel(new LevelExpression(LP)), truncParameter);
    SET_TRUNC = setTrunc.definition();
    setTrunc.addConstructor("inS", Abstract.Binding.DEFAULT_PRECEDENCE, new TypeUniverse(new LevelExpression(LP), new LevelExpression(LH)), param("inS", Reference(truncParameter)));
    ...
    Expression setTruncConParameter3type = FunCall(PATH_INFIX).applyLevelSubst(new LevelSubstitution(LP, new LevelExpression(LP), LH, new LevelExpression(0)))
    ...
    SET_TRUNC_PATH_CON = setTrunc.addConstructor("truncS", Abstract.Binding.DEFAULT_PRECEDENCE, new TypeUniverse(new LevelExpression(LP), new LevelExpression(LH)), setTruncConParameter1);
