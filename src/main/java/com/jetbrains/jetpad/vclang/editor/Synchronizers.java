...
public class Synchronizers {
  public static Synchronizer forDefinitions(Mapper<? extends Node, ? extends Cell> mapper, ObservableList<Definition> definitions, Cell target) {
    final WrapperContext ctx = mapper.getSource().getContext();
      ...
      public CompletionSupplier createRoleCompletion(Mapper<?, ?> mapper, Node contextNode, final Role<Definition> target) {
        return CompletionSupplier.create(new SimpleCompletionItem("fun ", "function") {
          ...
          public Runnable complete(String text) {
            return target.set(new FunctionDefinition(ctx));
          }
        });
      }
    });
    synchronizer.setItemFactory(new Supplier<Definition>() {
      ...
      public Definition get() {
        return new EmptyDefinition(ctx);
      }
    });
    return synchronizer;
  }
        ...
        return new CompletionSupplier() {
          ...
          public List<CompletionItem> get(CompletionParameters cp) {
            ...
            result.add(new SimpleCompletionItem("tele ", "telescope") {
              @Override
              public Runnable complete(String text) {
                return target.set(new Model.TelescopeArgument(ctx));
              }
            });
            return result;
          }
