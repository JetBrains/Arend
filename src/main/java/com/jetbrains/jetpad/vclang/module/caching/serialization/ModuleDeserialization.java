...
public class ModuleDeserialization {
  ...
  public ModuleDeserialization(ModuleScopeProvider moduleScopeProvider, TypecheckerState state, ErrorReporter errorReporter) {
    myModuleScopeProvider = moduleScopeProvider;
    myState = state;
    myErrorReporter = errorReporter;
  }

  public Group readModule(ModuleProtos.Module moduleProto, Function<ModulePath, Boolean> moduleLoader) throws DeserializationException {
    Group group = readGroup(moduleProto.getGroup(), null);
    ...
    for (ModuleProtos.ModuleCallTargets moduleCallTargets : moduleProto.getModuleCallTargetsList()) {
      ...
      if (scope == null) {
        myErrorReporter.report(new ModuleNotFoundError(module));
        return null;
      }
