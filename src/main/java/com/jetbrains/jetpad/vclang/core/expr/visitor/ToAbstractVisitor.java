...
public class ToAbstractVisitor extends BaseExpressionVisitor<Void, Abstract.Expression> implements ElimTreeNodeVisitor<Void, Abstract.Expression> {
  ...
  public Abstract.Expression visitConCall(ConCallExpression expr, Void params) {
    ...
    if (expr.getDefinition().status() != Definition.TypeCheckingStatus.TYPE_HAS_ERRORS && myFlags.contains(Flag.SHOW_CON_PARAMS) && (!expr.getDataTypeArguments().isEmpty() || myFlags.contains(Flag.SHOW_CON_DATA_TYPE))) {
  ...
  private String renameVar(Variable var) {
