...
public class CompareVisitor extends BaseExpressionVisitor<Expression, Boolean> {
  ...
  public Boolean compare(Expression expr1, Expression expr2) {
    ...
    if (infRefExpr1 != null) {
      return myNormalCompare && myEquations.addEquation(infRefExpr1, expr2.subst(getSubstitution()).normalize(NormalizeVisitor.Mode.WHNF), myCMP, infRefExpr1.getVariable().getSourceNode(), infRefExpr1.getVariable(), expr2.getStuckInferenceVariable(true));
    }
    if (infRefExpr2 != null) {
      return myNormalCompare && myEquations.addEquation(expr1.normalize(NormalizeVisitor.Mode.WHNF), infRefExpr2, myCMP, infRefExpr2.getVariable().getSourceNode(), expr1.getStuckInferenceVariable(true), infRefExpr2.getVariable());
    }

    InferenceVariable stuckVar1 = expr1.getStuckInferenceVariable(true);
    InferenceVariable stuckVar2 = expr2.getStuckInferenceVariable(true);
  ...
  private Boolean checkDefCallAndApp(Expression expr1, Expression expr2, boolean correctOrder) {
    ...
    while (true) {
      ...
      if (variable.isSolved()) {
        return compare(myNormalCompare ? myEquations : DummyEquations.getInstance(), myCMP, finalExpr1, finalExpr2, variable.getSourceNode());
      } else {
  ...
  private boolean checkClassCallSortArguments(ClassCallExpression classCall1, ClassCallExpression classCall2) {
    ...
    for (Map.Entry<ClassField, LamExpression> entry : classCall1.getDefinition().getImplemented()) {
      if (entry.getKey().hasUniverses() && !classCall2.isImplemented(entry.getKey())) {
        ...
        if (!compare(myNormalCompare ? myEquations : DummyEquations.getInstance(), Equations.CMP.LE, type, entry.getKey().getType(classCall2.getSortArgument()).applyExpression(thisExpr), mySourceNode)) {
          return false;
        }
      }
    }
  ...
  public Boolean visitUniverse(UniverseExpression expr1, Expression expr2) {
    return expr2.isInstance(UniverseExpression.class) && Sort.compare(expr1.getSort(), expr2.cast(UniverseExpression.class).getSort(), myCMP, myNormalCompare ? (myEquations == DummyEquations.getInstance() ? null : myEquations) : DummyEquations.getInstance(), mySourceNode);
  }
