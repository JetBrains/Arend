...
public class RecreateExpressionVisitor extends SubstVisitor {
  ...
  public Expression visitClassCall(ClassCallExpression expr, Void params) {
    ...
    ClassCallExpression classCall = new ClassCallExpression(expr.getDefinition(), expr.getSortArgument().subst(getLevelSubstitution()), fieldSet, expr.getSort().subst(getLevelSubstitution()), expr.getPLevelKind(), expr.getHLevelKind());
