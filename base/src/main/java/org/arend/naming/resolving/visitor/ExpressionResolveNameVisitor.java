...
public class ExpressionResolveNameVisitor extends BaseConcreteExpressionVisitor<Void> implements ExpressionResolver, ConcreteLevelExpressionVisitor<LevelVariable, Concrete.LevelExpression> {
  ...
  public static Referable resolve(Referable referable, Scope scope, boolean withArg, List<Referable> resolvedRefs, @Nullable Referable.RefKind kind) {
    ...
    if (referable instanceof UnresolvedReference) {
      if (withArg) {
        assert kind == ArendRef.RefKind.EXPR;
  ...
  public static Referable resolve(Referable referable, Scope scope) {
    return resolve(referable, scope, false, null, ArendRef.RefKind.EXPR);
  }
  ...
  public void finalizeReference(Concrete.BinOpSequenceElem<Concrete.Expression> elem, MetaBinOpParser.ResolvedReference resolvedReference) {
    ...
    Concrete.ReferenceExpression refExpr = resolvedReference.refExpr();
    if (refExpr.getReferent() instanceof UnresolvedReference) {
      resolve(refExpr, myScope, false, null);
    ...
    if (resolvedReference.resolvedList() != null && myResolverListener != null) {
      ...
      myResolverListener.referenceResolved(argument, resolvedReference.originalReference(), refExpr, resolvedReference.resolvedList(), myScope);
    }
  }
  ...
  private Concrete.Pattern visitPattern(Concrete.Pattern pattern, Map<String, Referable> usedNames) {
    ...
    if (pattern instanceof Concrete.NamePattern namePattern) {
      ...
      Concrete.NamePattern result = new Concrete.NamePattern(pattern.getData(), namePattern.isExplicit(), local, ((Concrete.NamePattern) pattern).type);
  ...
  public void resolvePattern(Concrete.Pattern pattern) {
    ...
    if (origReferable instanceof UnresolvedReference) {
      List<Referable> resolvedList = myResolverListener == null ? null : new ArrayList<>();
      Referable referable = convertChecked(resolve(origReferable, myParentScope, false, resolvedList, ArendRef.RefKind.EXPR), ((UnresolvedReference) origReferable).getData());
  ...
  Referable visitClassFieldReference(Concrete.ClassElement element, Referable oldField, ClassReferable classDef) {
    if (oldField instanceof UnresolvedReference) {
      ...
      Referable field = resolve(oldField, new MergeScope(true, new ClassFieldImplScope(classDef, true), myScope), false, resolvedRefs, ArendRef.RefKind.EXPR);
      if (myResolverListener != null) {
  ...
  public Concrete.LevelExpression visitVar(Concrete.VarLevelExpression expr, LevelVariable type) {
    Referable.RefKind refKind = type == LevelVariable.HVAR ? ArendRef.RefKind.HLEVEL : ArendRef.RefKind.PLEVEL;
