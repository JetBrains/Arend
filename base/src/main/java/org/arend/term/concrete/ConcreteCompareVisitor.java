...
public class ConcreteCompareVisitor implements ConcreteExpressionVisitor<Concrete.Expression, Boolean>, ConcreteResolvableDefinitionVisitor<Concrete.ResolvableDefinition, Boolean> {
  ...
  private boolean compareLevels(List<Concrete.LevelExpression> levels1, List<Concrete.LevelExpression> levels2) {
    if (levels1 == null || levels2 == null) return levels1 == levels2;
  ...
  @Override
  public Boolean visitCore(Concrete.CoreExpression expr, Concrete.Expression expr2) {
    return expr2 instanceof Concrete.CoreExpression && expr.getTypedExpression() == ((Concrete.CoreExpression) expr2).getTypedExpression();
  }
  ...
  private boolean compareConstructor(Concrete.Constructor con1, Concrete.Constructor con2) {
    ...
    boolean result = con1.getData().getRefName().equals(con2.getData().getRefName()) && compareParameters(con1.getParameters(), con2.getParameters()) && compare(con1.getResultType(), con2.getResultType()) && compareExpressionList(con1.getEliminatedReferences(), con2.getEliminatedReferences()) && compareClauses(con1.getClauses(), con2.getClauses());
