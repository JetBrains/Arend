...
public class RecursiveInstanceInferenceError extends InstanceInferenceError {
  ...
  private RecursiveInstanceInferenceError(ExpressionPrettifier prettifier, ArendRef classRef, CoreExpression expected, CoreExpression actual, CoreExpression classifyingExpression, ConcreteSourceNode cause, RecursiveInstanceHoleExpression holeExpr, CoreExpression[] candidates) {
    super(prettifier, classRef, expected, actual, classifyingExpression, holeExpr != null && holeExpr.getData() instanceof ConcreteSourceNode ? (ConcreteSourceNode) holeExpr.getData() : cause, candidates);
    this.holeExpression = holeExpr;
  }

  public RecursiveInstanceInferenceError(ExpressionPrettifier prettifier, ArendRef classRef, ConcreteSourceNode cause, RecursiveInstanceHoleExpression holeExpr, CoreExpression[] candidates) {
    this(prettifier, classRef, null, null, null, cause, holeExpr, candidates);
  }
  ...
  public RecursiveInstanceInferenceError(ArendRef classRef, CoreExpression classifyingExpression, ConcreteSourceNode cause, RecursiveInstanceHoleExpression holeExpr) {
    this(null, classRef, null, null, classifyingExpression, cause, holeExpr, new CoreExpression[0]);
  }
  ...
  public RecursiveInstanceInferenceError(ArendRef classRef, CoreExpression expected, CoreExpression actual, ConcreteSourceNode cause, CoreExpression candidate, RecursiveInstanceHoleExpression holeExpr) {
    this(null, classRef, expected, actual, null, cause, holeExpr, new CoreExpression[1]);
  ...
  public void addAdditionalDocs(List<Doc> docs, PrettyPrinterConfig ppConfig) {
    if (holeExpression != null) {
      for (int i = holeExpression.recursiveData.size() - 1; i >= 0; i--) {
        ...
        docs.add(withExpr ? hang(line, termDoc(recursiveData.classifyingExpression, prettifier, ppConfig)) : line);
      }
    }
  }
